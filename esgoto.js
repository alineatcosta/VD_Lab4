...
var svg3 = d3.select("#chartinformacao3")
  .append("svg")
  .attr('version', '1.1')
...
function informacoesEsgoto() {
  var color = d3.scaleThreshold();
  var svg3 = d3.select("#chartinformacao3");

  d3.csv("data/indicadores_operacionais_esgoto.csv", function(error, informacao) {
    ...
    for (var i in informacao) {
        if (informacao[i].tratamento_de_esgoto_percentual !== "NA") {
          svg3.select(".municipio_"+informacao[i].codigo_municipio)
            .attr("fill", color(+informacao[i].tratamento_de_esgoto_percentual));
        } else {
          svg3.select(".municipio_"+informacao[i].codigo_municipio)
            .attr("fill", "#708090");
        }
  ...
  d3.csv("data/municipios_sab.csv", function(error, informacao) {
    ...
    for (var i in informacao) {
      if (informacao[i].GEOCODIGO) {
        svg3.select("#municipio_"+informacao[i].GEOCODIGO)
